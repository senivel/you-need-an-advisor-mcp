...
Tool functions are being migrated to domain modules under ``ynab_mcp.tools/``.
The FastMCP instance, AppContext, and lifespan now live in ``ynab_mcp.app``.
"""
...
# Import tool, resource, and prompt modules to register their decorators on `mcp`.
# Must be at bottom to avoid circular imports (these modules import `mcp` from app).
