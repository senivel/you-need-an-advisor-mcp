...
class TestNoBudgetIdProvided:
    ...
    @pytest.mark.anyio
    async def test_single_budget_returns_that_budget_id(self):
        """Single budget auto-selects and returns its ID."""
        client = _mock_client([BUDGET_A])
        budget_id, _message = await resolve_budget(client)

        assert budget_id == "aaaa-1111"

    @pytest.mark.anyio
    async def test_single_budget_message_includes_name(self):
    ...
    @pytest.mark.anyio
    async def test_multiple_budgets_raises_tool_error(self):
        """Multiple budgets without ID raises ToolError listing all budgets."""
        client = _mock_client([BUDGET_A, BUDGET_B])
    ...
    @pytest.mark.anyio
    async def test_no_budgets_raises_tool_error(self):
...
class TestExactUUIDMatch:
    ...
    @pytest.mark.anyio
    async def test_exact_uuid_match_returns_budget_id(self):
...
class TestFuzzyNameMatch:
    ...
    @pytest.mark.anyio
    async def test_low_similarity_raises_tool_error(self):
